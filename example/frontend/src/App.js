...
const Input = ({ input, meta, label, ...rest }) => {
  return <div className={`${meta.error && "error"} field`}>
    <label>{label}</label>
    <input {...input} {...rest} />
  </div>
}
...
class App extends PureComponent {
  ...
  render() {
    ...
    return <main className={`ui form container card ${invalid && "error"}`}>
      <div className="content">
        <div className="header">Product</div>
        <p className="meta">{this.basePrice}€/unit</p>

        <Field component={Input} label="Quantity" placeholder="1" name="amount" type="number" onChange={this.onChange} />
        <Field component={Input} label="Price" placeholder={`${this.basePrice}€`} name="price" type="text" disabled />

        { errors &&
          <div className="ui error message">
            {Object.keys(errors).map(input =>
              errors[input].map((error, i) =>
                <p key={`${input}-${i}`}>{ error }</p>
              )
            )}
          </div>
        }
